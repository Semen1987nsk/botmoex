...
async def cmd_scan(message: Message):
    ...
    await message.answer(
        ...
        f"<i>Сигналы будут приходить при НОВЫХ пробоях канала ±4.5σ</i>",
        parse_mode='HTML'
    )
    ...
    for ticker in test_tickers:
        ...
        try:
            ...
            results.append(
                ...
                f"  ⬆️ Верх (+4.5σ): {channel['upper']:.2f}\n"
                f"  ⬇️ Низ (-4.5σ): {channel['lower']:.2f}\n"
...
async def format_extremes_message(extremes_up, extremes_down):
    ...
    if not extremes_up and not extremes_down:
        return "✅ <b>Все инструменты внутри каналов ±4.5σ</b>"
    
    lines = ["📊 <b>ТЕКУЩИЕ ПОЗИЦИИ ЗА ПРЕДЕЛАМИ ±4.5σ</b>\n"]
    
    if extremes_up:
        lines.append(f"\n🔺 <b>ВЫШЕ +4.5σ ({len(extremes_up)}):</b>")
    ...
    if extremes_down:
        lines.append(f"\n🔻 <b>НИЖЕ -4.5σ ({len(extremes_down)}):</b>")
...
async def send_periodic_summary():
    """Отправить сводку инструментов за пределами ±4.5σ."""
    if not subscribers:
        return
    ...
    if above_list:
        ...
        lines.append(f"🔺 <b>ВЫШЕ +4.5σ ({len(above_list)}):</b>")
    ...
    if below_list:
        ...
        lines.append(f"🔻 <b>НИЖЕ -4.5σ ({len(below_list)}):</b>")
...
async def send_signal(ticker, data, price, signal_type):
    ...
    message = (
        ...
        f"⬆️ Верх (+4.5σ): {data['upper']:.2f}\n"
        f"⬇️ Низ (-4.5σ): {data['lower']:.2f}\n"
