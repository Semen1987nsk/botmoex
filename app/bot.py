...
# ticker -> {figi, name, type, board, engine, market, upper, lower, last_signal_type, closes_299}
instruments = {}
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "Мониторинг пробоев линейных регрессионных каналов (4.5 STD) на 10-минутках.\n\n"
        "Команды:\n"
...
async def cmd_scan(message: Message):
    ...
    await message.answer(
        ...
        f"<i>Сигналы будут приходить при НОВЫХ пробоях канала ±3.8σ</i>",
        parse_mode='HTML'
    )
    ...
    for ticker in test_tickers:
        ...
        try:
            ...
            results.append(
                ...
                f"  ⬆️ Верх (+3.8σ): {channel['upper']:.2f}\n"
                f"  ⬇️ Низ (-3.8σ): {channel['lower']:.2f}\n"
...
async def format_extremes_message(extremes_up, extremes_down):
    ...
    if not extremes_up and not extremes_down:
        return "✅ <b>Все инструменты внутри каналов ±3.8σ</b>"
    
    lines = ["📊 <b>ТЕКУЩИЕ ПОЗИЦИИ ЗА ПРЕДЕЛАМИ ±3.8σ</b>\n"]
    
    if extremes_up:
        lines.append(f"\n🔺 <b>ВЫШЕ +3.8σ ({len(extremes_up)}):</b>")
    ...
    if extremes_down:
        lines.append(f"\n🔻 <b>НИЖЕ -3.8σ ({len(extremes_down)}):</b>")
    ...
    try:
        ...
        if len(df) >= REGRESSION_LENGTH:
            ...
            if channel:
                ...
                instruments[ticker]['closes_299'] = closes[-(REGRESSION_LENGTH-1):]
...
async def send_periodic_summary():
    """Отправить сводку инструментов за пределами ±3.8σ."""
    if not subscribers:
        return
    ...
    if above_list:
        ...
        lines.append(f"🔺 <b>ВЫШЕ +3.8σ ({len(above_list)}):</b>")
    ...
    if below_list:
        ...
        lines.append(f"🔻 <b>НИЖЕ -3.8σ ({len(below_list)}):</b>")
...
async def check_breakout(ticker, current_price):
    ...
    if closes_299 is None or len(closes_299) < REGRESSION_LENGTH - 1:
        ...
        channel = calculate_linreg_realtime(closes_299, current_price, STD_DEV_MULTIPLIER)
        upper = channel['upper']
        lower = channel['lower']
...
async def send_signal(ticker, data, price, signal_type):
    ...
    message = (
        ...
        f"⬆️ Верх (+3.8σ): {data['upper']:.2f}\n"
        f"⬇️ Низ (-3.8σ): {data['lower']:.2f}\n"
